...
def executive_summary_agent(state: ScannerState) -> dict:
    """
    reporting agent: executive_summary_agent
    """
    llm = get_llm()
    ...
    system_prompt = """Sen bir C-Level Yöneticiler için Siber Güvenlik Analistisin.
Görevin tüm bulguları inceleyip, zafiyetlerin "iş etkisini" (business impact) anlatan teknik olmayan, risk odaklı bir özet rapor yazmaktır.
...
Bulgularını aşağıdaki JSON formatında döndürmelisin:
    ...
    try:
        ...
        print(f"[executive_summary_agent] JSON parse hatası: {e}")
    ...
    if report_update:
        ...
        current_report += f"\n\n=== EXECUTIVE_SUMMARY_AGENT GÜNCELLEMESİ ===\n" + report_update
