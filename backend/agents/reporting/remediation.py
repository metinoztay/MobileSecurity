...
def remediation_agent(state: ScannerState) -> dict:
    """
    reporting agent: remediation_agent
    """
    llm = get_llm()
    ...
    system_prompt = """Sen bir Çözüm (Remediation) ve Güvenli Kodlama Uzmanısın.
Görevin tespit edilen zafiyetlerin nasıl giderileceğine dair geliştiriciler için spesifik kod parçacıkları (fix code) ve adımlar üretmektir.
...
Bulgularını aşağıdaki JSON formatında döndürmelisin:
    ...
    try:
        ...
        print(f"[remediation_agent] JSON parse hatası: {e}")
    ...
    if report_update:
        ...
        current_report += f"\n\n=== REMEDIATION_AGENT GÜNCELLEMESİ ===\n" + report_update
