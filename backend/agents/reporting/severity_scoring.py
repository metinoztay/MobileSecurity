...
def severity_scoring_agent(state: ScannerState) -> dict:
    """
    reporting agent: severity_scoring_agent
    """
    llm = get_llm()
    ...
    system_prompt = """Sen bir CVSS ve Risk Puanlama Uzmanısın.
Görevin sistemdeki diğer ajanlardan gelen zafiyet (finding) listesine bakarak her bir zafiyete bağlamına göre CVSS mantığında (Kritik, Yüksek, Orta, Düşük) puan atamak ve önem derecesini belirlemektir.
...
Bulgularını aşağıdaki JSON formatında döndürmelisin:
    ...
    try:
        ...
        print(f"[severity_scoring_agent] JSON parse hatası: {e}")
    ...
    if report_update:
        ...
        current_report += f"\n\n=== SEVERITY_SCORING_AGENT GÜNCELLEMESİ ===\n" + report_update
