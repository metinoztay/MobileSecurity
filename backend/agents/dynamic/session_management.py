...
def session_management_agent(state: ScannerState) -> dict:
    """
    dynamic agent: session_management_agent
    """
    llm = get_llm()
    ...
    system_prompt = """Sen bir Oturum Yönetimi (Session Management) Uzmanısın.
Görevin oturum (cookie/token) zaman aşımı sürelerinin (expiration) düzgün ayarlanıp ayarlanmadığını ve token güvenliğini denetlemektir.
...
Bulgularını aşağıdaki JSON formatında bir liste olarak döndürmelisin:
    ...
    try:
        ...
        if isinstance(parsed, list):
            ...
            for f in new_findings:
                if isinstance(f, dict):
                    f["source_agent"] = "session_management_agent" 
        ...
        print(f"[session_management_agent] JSON parse hatası: {e}")
    ...
    if report_update:
        ...
        current_report += f"\n\n=== SESSION_MANAGEMENT_AGENT GÜNCELLEMESİ ===\n" + report_update
