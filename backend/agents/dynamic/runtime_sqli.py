...
def runtime_sqli_agent(state: ScannerState) -> dict:
    """
    dynamic agent: runtime_sqli_agent
    """
    llm = get_llm()
    ...
    system_prompt = """Sen bir Dinamik SQL Enjeksiyon Uzmanısın.
Görevin ağ isteklerinden dönen HTTP yanıtlarında veritabanı hataları veya SQLi payload'larının manipüle ettiği verileri bulmaktır.
...
Bulgularını aşağıdaki JSON formatında bir liste olarak döndürmelisin:
    ...
    try:
        ...
        if isinstance(parsed, list):
            ...
            for f in new_findings:
                if isinstance(f, dict):
                    f["source_agent"] = "runtime_sqli_agent" 
        ...
        print(f"[runtime_sqli_agent] JSON parse hatası: {e}")
    ...
    if report_update:
        ...
        current_report += f"\n\n=== RUNTIME_SQLI_AGENT GÜNCELLEMESİ ===\n" + report_update
