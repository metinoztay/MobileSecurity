...
def data_leakage_agent(state: ScannerState) -> dict:
    """
    dynamic agent: data_leakage_agent
    """
    llm = get_llm()
    ...
    system_prompt = """Sen bir Veri Sızıntısı Uzmanısın.
Görevin uygulama çalışırken Logcat çıktılarına, bellek analizine veya sistem panosuna (clipboard) hassas bilgilerin yazılıp yazılmadığını tespit etmektir.
...
Bulgularını aşağıdaki JSON formatında bir liste olarak döndürmelisin:
    ...
    try:
        ...
        if isinstance(parsed, list):
            ...
            for f in new_findings:
                if isinstance(f, dict):
                    f["source_agent"] = "data_leakage_agent" 
        ...
        print(f"[data_leakage_agent] JSON parse hatası: {e}")
    ...
    if report_update:
        ...
        current_report += f"\n\n=== DATA_LEAKAGE_AGENT GÜNCELLEMESİ ===\n" + report_update
