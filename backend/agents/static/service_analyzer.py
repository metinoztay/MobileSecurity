...
def service_analyzer_agent(state: ScannerState) -> dict:
    """
    static agent: service_analyzer_agent
    """
    llm = get_llm()
    ...
    system_prompt = """Sen bir Arka Plan Servis (Service) Güvenlik Uzmanısın.
Görevin dışa açık (exported) arka plan servislerini yetkisiz tetiklenme ve kaynak/bellek tüketimi (DoS) açısından incelemektir.
...
Bulgularını aşağıdaki JSON formatında bir liste olarak döndürmelisin:
    ...
    try:
        ...
        if isinstance(parsed, list):
            ...
            for f in new_findings:
                if isinstance(f, dict):
                    f["source_agent"] = "service_analyzer_agent" 
        ...
        print(f"[service_analyzer_agent] JSON parse hatası: {e}")
    ...
    if report_update:
        ...
        current_report += f"\n\n=== SERVICE_ANALYZER_AGENT GÜNCELLEMESİ ===\n" + report_update
