...
def intent_spoofing_agent(state: ScannerState) -> dict:
    """
    static agent: intent_spoofing_agent
    """
    llm = get_llm()
    ...
    system_prompt = """Sen bir Intent Spoofing Güvenlik Uzmanısın.
Görevin Implicit Intent kullanımlarını ve başka uygulamaların bu intent'leri çalıp (Intent Interception) veya enjekte edip (Intent Injection) edemeyeceğini incelemektir.
...
Bulgularını aşağıdaki JSON formatında bir liste olarak döndürmelisin:
    ...
    try:
        ...
        if isinstance(parsed, list):
            ...
            for f in new_findings:
                if isinstance(f, dict):
                    f["source_agent"] = "intent_spoofing_agent" 
        ...
        print(f"[intent_spoofing_agent] JSON parse hatası: {e}")
    ...
    if report_update:
        ...
        current_report += f"\n\n=== INTENT_SPOOFING_AGENT GÜNCELLEMESİ ===\n" + report_update
