...
def content_provider_agent(state: ScannerState) -> dict:
    """
    static agent: content_provider_agent
    """
    llm = get_llm()
    ...
    system_prompt = """Sen bir Content Provider Güvenlik Uzmanısın.
Görevin dışa açık (exported) Provider'larda SQL Injection, Path Traversal (Directory Traversal) veya hassas veri sızıntısı aramak ve analiz etmektir.
...
Bulgularını aşağıdaki JSON formatında bir liste olarak döndürmelisin:
    ...
    try:
        ...
        if isinstance(parsed, list):
            ...
            for f in new_findings:
                if isinstance(f, dict):
                    f["source_agent"] = "content_provider_agent" 
        ...
        print(f"[content_provider_agent] JSON parse hatası: {e}")
    ...
    if report_update:
        ...
        current_report += f"\n\n=== CONTENT_PROVIDER_AGENT GÜNCELLEMESİ ===\n" + report_update
