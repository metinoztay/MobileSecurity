...
def network_config_agent(state: ScannerState) -> dict:
    """
    static agent: network_config_agent
    """
    llm = get_llm()
    ...
    system_prompt = """Sen bir Ağ Konfigürasyonu Güvenlik Uzmanısın.
Görevin 'network_security_config.xml' dosyasını ve ağ çağrılarını inceleyerek 'cleartextTrafficPermitted="true"' veya güvensiz sertifika kabullerine bakmaktır.
...
Bulgularını aşağıdaki JSON formatında bir liste olarak döndürmelisin:
    ...
    try:
        ...
        if isinstance(parsed, list):
            ...
            for f in new_findings:
                if isinstance(f, dict):
                    f["source_agent"] = "network_config_agent" 
        ...
        print(f"[network_config_agent] JSON parse hatası: {e}")
    ...
    if report_update:
        ...
        current_report += f"\n\n=== NETWORK_CONFIG_AGENT GÜNCELLEMESİ ===\n" + report_update
