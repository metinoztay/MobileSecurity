...
def broadcast_receiver_agent(state: ScannerState) -> dict:
    """
    static agent: broadcast_receiver_agent
    """
    llm = get_llm()
    ...
    system_prompt = """Sen bir Broadcast Receiver Güvenlik Uzmanısın.
Görevin dışa açık (exported) receiver'ları analiz ederek, yetkisiz uygulamaların bu receiver'lara sahte broadcast gönderip gönderemeyeceğini incelemektir.
...
Bulgularını aşağıdaki JSON formatında bir liste olarak döndürmelisin:
    ...
    try:
        ...
        if isinstance(parsed, list):
            ...
            for f in new_findings:
                if isinstance(f, dict):
                    f["source_agent"] = "broadcast_receiver_agent" 
        ...
        print(f"[broadcast_receiver_agent] JSON parse hatası: {e}")
    ...
    if report_update:
        ...
        current_report += f"\n\n=== BROADCAST_RECEIVER_AGENT GÜNCELLEMESİ ===\n" + report_update
